    ...
    for root, dirs, files in os.walk(directorio_objetos):
        for nombre_archivo in files:
            if nombre_archivo.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                relative_path = os.path.relpath(objeto_path, os.path.dirname(os.path.dirname(directorio_objetos)))
                ...
                # Guardar salidas en directorios replicando estructura relativa
...
def leer_rutas_desde_txt(archivo_txt):
    ...
    Cada línea del archivo debe contener una ruta válida.
    """
    with open(archivo_txt, 'r') as file:
        rutas = [line.strip() for line in file if line.strip()]
    return rutas

# Parámetros
archivo_rutas = 'input/rutas_directorios.txt'  # Archivo .txt con las rutas
prefix = 'input/'  # Prefijo para las rutas de entrada
directorio_salida = prefix + 'salida'  # Directorio para guardar las imágenes procesadas
