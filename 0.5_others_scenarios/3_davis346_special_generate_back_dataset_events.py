...
def leer_rutas_desde_txt(archivo_txt):
    ...
    with open(archivo_txt, 'r') as file:
        for line in file:
            ruta = line.strip()
            if (
                any(palabra in ruta for palabra in palabras_clave_1)
            ):
                rutas_filtradas.append(ruta)
    return rutas_filtradas

# Parámetros
archivo_rutas = 'input/txt/events_folders.txt'  # Archivo .txt con las rutas
prefix = 'input/'  # Prefijo para las rutas de entrada
directorio_salida =  'input/frames_all_with_back_without_blur/'  # Directorio para guardar las imágenes procesadas
